...
<script>
	...
	export default {
		...
		methods: {
		    ...
		    async getElementData(el, callback){
				await uni.createSelectorQuery().in(this).selectAll(el).boundingClientRect().exec((data) => {
					callback(data[0]);
				});
			},
			scroll(e) {
		...
		.tab {
			...
			&__item {
				...
				&--active {
					...
					color: #1E2128;
					border-radius: 6rpx;
					border: 4rpx solid #FAE39D!important;
				}
				&-title {
